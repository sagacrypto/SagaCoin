...
bool parseBitcoinURI(const QUrl &uri, SendCoinsRecipient *out)
{
    ...
    if(uri.scheme() != QString("SagaCoin"))
        return false;
...
{
    // Convert SagaCoin:// to SagaCoin:
    //
    //    Cannot handle this later, because bitcoin:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("SagaCoin://"))
    {
        uri.replace(0, 12, "SagaCoin:");
    }
...
{
    return GetSpecialFolderPath(CSIDL_STARTUP) / "SagaCoin.lnk";
}
...
{
    return GetAutostartDir() / "SagaCoin.desktop";
}
...
HelpMessageBox::HelpMessageBox(QWidget *parent) :
    QMessageBox(parent)
{
    header = tr("SagaCoin-Qt") + " " + tr("version") + " " +
        QString::fromStdString(FormatFullVersion()) + "\n\n" +
        tr("Usage:") + "\n" +
        "  SagaCoin-qt [" + tr("command-line options") + "]                     " + "\n";
